...
namespace InventoryTooltips
{
    internal class InventoryTooltipsPatches
    {
        ...
        private static string GetBuyPrice(InventorySlot slot)
        {
            ...
            if (Plugin.instance.GetShowStackPrice() && itemInSlot.isStackable && !itemInSlot.isATool && !itemInSlot.isPowerTool && !itemInSlot.hasFuel)
            {
                amount *= slot.stack;
            }
        ...
        private static string GetSellPrice(InventorySlot slot)
        {
            ...
            if (displayType == DisplayType.WithLicence)
            {
